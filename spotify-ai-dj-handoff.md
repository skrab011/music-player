...
- **The Spotify app caches the library — newly created playlists may not appear until you force-restart the app.** Happened building the 7: all runs succeeded and the playlists existed (update-mode found them by name), but the phone showed nothing until Jacob fully restarted Spotify. Not a bug. `push_playlist.py` now prints the target account (display name + id) and a direct `open.spotify.com/playlist/<id>` link on every run, so "not showing up" is instantly diagnosable — click the link before assuming anything's broken.
